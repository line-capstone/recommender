...
* 문서 요약\(document summarization\)에 적용할 수 있다. 한 문장을 트랜잭션 데이터로 상정하고, 문장 내 출현한 단어를 아이템으로 놓고 같은 방식으로 분석할 수 있다. 이를 통해 도출된 유의미한 규칙들은 단어의 co-occurence 정보를 함축할 것이다. 예를 들어 조건절이 {발, 없는, 말이}라면, 결과절은 {간다}가 나오는 것과 같은 방식이다.
